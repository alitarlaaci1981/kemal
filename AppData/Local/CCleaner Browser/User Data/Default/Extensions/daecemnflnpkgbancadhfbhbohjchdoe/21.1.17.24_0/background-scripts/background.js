...
/**
 * Check if CCleaner HTTPS Encryption should be ON for host
 */
...
function updateState () {
  ...
  chrome.browserAction.setTitle({
    title: 'CCleaner HTTPS Encryption' + ((iconState === 'active') ? '' : ' (' + iconState + ')')
  });
...
function onBeforeRequest(details) {
  ...
  if (details.type == "main_frame") {
    ...
    // Check if an user has disabled CCleaner HTTPS Encryption on this site.  We should
    // ensure that all subresources are not run through CCleaner HTTPS Encryption as well.
